...
def generate_synthetic_data(n_dim, n_train, n_test, n_categ, dist_type):
    assert n_train%n_categ==0
    assert n_test%n_categ==0
    X_train = []
    y_train = []
    X_test = []
    y_test = []
    n_train_categ = n_train/n_categ
    n_test_categ = n_test/n_categ
    if dist_type == 0:
        beta = 1.
        for i in range(n_categ):
            X_train.append(exprv(n_train_categ, n_dim,beta))
            X_test.append(exprv(n_test_categ, n_dim,beta))
            y_train += [i]*n_train_categ
            y_test += [i]*n_test_categ
            beta += 1.
    return (np.asarray(X_train).reshape(n_train,n_dim), np.asarray(y_train).reshape(n_train)), (np.asarray(X_test).reshape(n_test,n_dim), np.asarray(y_test).reshape(n_test))
...
(x_train, y_train), (x_test, y_test) = generate_synthetic_data(n_dim, n_train, n_test, n_categ, dist_type)
...
inputs_ = Input(shape=(100,))
od_1 = Dense(64, kernel_regularizer=OrthogonalL1L2(ortho_l1, ortho_l2))
od_2 = Dense(32, kernel_regularizer=OrthogonalL1L2(ortho_l1, ortho_l2))
od_3 = Dense(16, kernel_regularizer=OrthogonalL1L2(ortho_l1, ortho_l2))
od_4 = Dense(8, kernel_regularizer=OrthogonalL1L2(ortho_l1, ortho_l2))
od_5 = Dense(8, kernel_regularizer=OrthogonalL1L2(ortho_l1, ortho_l2))
...
out_6 = DenseTranspose(od_5, 8)(out_5)
out_7 = DenseTranspose(od_4, 16)(out_6)
out_8 = DenseTranspose(od_3, 32)(out_7)
out_9 = DenseTranspose(od_2, 64)(out_8)
out_10 = DenseTranspose(od_1, 100)(out_9)

model = Model(inputs=inputs_, outputs=out_10)

model.compile(loss='mean_absolute_error',
              optimizer="adam",
              metrics=['mean_absolute_error'])

hidden_fn = KK.function([inputs_], out_5)
...
model.fit(x_train, x_train,
            ...
            callbacks=[ModelCheckpoint(wts_name, monitor='mean_squared_error', verbose=1, save_best_only=True, save_weights_only=True, mode='auto', period=1)])

model.load_weights(wts_name)
score = model.evaluate(x_test, x_test, verbose=1)
print('Test loss:', score[0])
print('Test mean_absolute_error:', score[1])

z_train = hidden_fn([x_train])
z_test = hidden_fn([x_test])
...
def compute_P_z(num_classes, y_train, z_train, y_test, z_test, N_moments=5):
    ...
    for c in range(num_classes):
        ...
        range_z[c] = (np.min(z_train[y_train==c,:], axis=0)*0-1./(c+1.)+1e-6, np.max(z_train[y_train==c,:], axis=0)*0+1./(c+1.)-1e-6)
    x_C_y = xCy(N_moments)
